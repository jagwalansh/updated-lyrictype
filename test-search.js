...
async function run() {
  ...
  const results = (itunesData.results || []).map((item) => {
    ...
    const match = lrcData.find((lrcItem) => {
      ...
      return (
        (lTrack.includes(tNameNorm) || tNameNorm.includes(lTrack)) &&
    ...
    return {
      ...
      hasLyrics: !!match,
      matchReason: match
        ? `Matched with lrclib: ${match.trackName} - ${match.artistName}`
        : "No match in lrclib search",
    };
  });
  ...
  withoutLyrics
    .slice(0, 10)
    .forEach((r) => console.log(`${r.track} - ${r.artist} | ${r.matchReason}`));
}
